...
from bank_system.decorators.rent_analyst_decorator import RentAnalystDecorator
...
class RentAnalystDecoratorSpec(unittest.TestCase):
    ...
    def it_creates_a_loan_request(self):
        ...
        an_employee_decorator.decorate(self.a_person)
        self.a_rent_analyst_decorator.decorate(self.a_person)
        self.a_rent_analyst_decorator.create_loan_request(self.an_account, 10000)
    ...
    def it_analyses_a_loan_request(self):
        ...
        self.a_rent_analyst_decorator.decorate(self.a_person)
        self.an_account.average_rent = 5000
        #should approve
        self.a_rent_analyst_decorator.create_loan_request(self.an_account, 10000)
        self.a_rent_analyst_decorator.analyse(self.an_account.number)
        self.a_rent_analyst_decorator.decorated.output_area['1234567-8'].approved |should| equal_to(True)
        #should refuse
        self.a_rent_analyst_decorator.create_loan_request(self.an_account, 50000)
        self.a_rent_analyst_decorator.analyse(self.an_account.number)
    ...
    def it_creates_a_loan(self):
        an_employee_decorator = EmployeeDecorator()
        an_employee_decorator.decorate(self.a_person)
        loan_request = LoanRequest(self.an_account, 7000, self.a_rent_analyst_decorator)
        self.a_rent_analyst_decorator.decorate(self.a_person)
        self.a_rent_analyst_decorator.decorated.output_area[self.an_account.number] = loan_request
        ...
        #creates the loan
        self.a_rent_analyst_decorator.create_loan(loan_request)
        #loan key is the analyst's register
        self.a_rent_analyst_decorator.decorated.output_area.values() |should| have_at_least(1).loan
    ...
    def it_moves_the_loan_to_an_account(self):
        ...
        (self.a_rent_analyst_decorator.move_loan_to_account, passing_a_wrong_key, self.an_account) |should| throw(KeyError)
        passing_a_non_account = 'I am not an account'
        (self.a_rent_analyst_decorator.move_loan_to_account, self.an_account.number, passing_a_non_account) |should| throw(ContractError)
        #prepares the account
        a_machine = Machine()
        self.an_account.decorate(a_machine)
        #should work
        loan_key = self.a_rent_analyst_decorator.register
        self.a_rent_analyst_decorator.move_loan_to_account(loan_key, self.an_account)
    ...
    def it_changes_its_loan_limit(self):
        self.a_rent_analyst_decorator.change_loan_limit(100000)
        self.a_rent_analyst_decorator.loan_limit |should| be(100000)
